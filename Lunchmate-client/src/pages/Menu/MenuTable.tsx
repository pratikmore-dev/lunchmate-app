...
useEffect(() => {
  ...
  const foundVendor  = vendor.find(v => v.vendorID === vendorID);
  setSelectedVendor(foundVendor || null);
  
  getVendorMenu(vendorID)
    .then((res) => {
      // Don't add quantity/selectedRate here - that's in cart context now
...
// New handlers that work with cart context
const handleQuantityChange = (menuItem: VendorMenuItem, change: number) => {
  const cartItem = getItemInCart(menuItem.vendorMenuID);
  
  if (!cartItem && change > 0) {
    // Item not in cart, need to add it
    addToCart({
      vendorMenuID: menuItem.vendorMenuID,
      menuID: menuItem.menuID,
      menuName: menuItem.menuName,
      vendorID: vendorID,
      vendorName: menuItem.vendorName || 'Unknown Vendor',
      fullRate: menuItem.fullRate,
      halfRate: menuItem.halfRate,
      vendorSpecificNotes: menuItem.vendorSpecificNotes,
    });
    return;
  }
  ...
  if (cartItem) {
    ...
    updateQuantity(menuItem.vendorMenuID, Math.max(0, newQuantity));
  }
};

const handleRateSelection = (vendorMenuID: string, rate: 'full' | 'half') => {
  updateRate(vendorMenuID, rate);
};

// Helper to get current quantity for display
const getDisplayQuantity = (vendorMenuID: string): number => {
  const cartItem = getItemInCart(vendorMenuID);
  return cartItem?.quantity || 0;
};

// Helper to get current rate for display
const getDisplayRate = (vendorMenuID: string): 'full' | 'half' => {
  const cartItem = getItemInCart(vendorMenuID);
  return cartItem?.selectedRate || 'full';
};
...
<TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
  {menuItems.length === 0 ? (
    ...
    menuItems.map((item) => {
      // Get current cart state for this item
      const displayQuantity = getDisplayQuantity(item.vendorMenuID);
      const displayRate = getDisplayRate(item.vendorMenuID);
          ...
          <TableCell className="px-4 py-3 text-start">
            <div className="flex items-center gap-2">
              <button
                ...
                disabled={displayQuantity === 0 || !item.isAvailable}
              ...
              <input
                ...
                onChange={(e) => {
                  ...
                  if (value >= 0) {
                    const cartItem = getItemInCart(item.vendorMenuID);
                    if (cartItem) {
                      updateQuantity(item.vendorMenuID, value);
                    } else if (value > 0) {
                      // Add to cart if typing a number directly
...
<div className="fixed bottom-8 right-8 z-50">
  <div className="group relative">
    ...
    <button
      onClick={openModal}
      className="relative flex h-14 w-14 items-center justify-center rounded-full bg-brand-600 text-white shadow-lg transition-all hover:bg-brand-700 hover:shadow-xl active:scale-95"
    >
      <svg
        width="20"
        height="20"
        ...
        <tr key={item.vendorMenuID}>
          ...
          <td className="px-3 py-2 text-sm text-gray-900 dark:text-white">
            <div>
              ...
              {/* Show vendor name instead of notes */}
